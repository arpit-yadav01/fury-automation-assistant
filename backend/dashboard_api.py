...
def _parse_natural(command):
    ...
    Returns (type, data) or (None, None) if not recognized.

    Examples:
      "say hi to Rishu on whatsapp"  → ("whatsapp_send", {contact, message})
      "send hello to John on whatsapp" → ("whatsapp_send", {contact, message})
      "play lofi on youtube"          → ("youtube", {query})
      "open two sum on leetcode"      → ("leetcode", {problem})
      "search react jobs on naukri"   → ("job_search", {query, platform})
    """
    cmd = command.lower().strip()

    # ---- WHATSAPP patterns ----
    # "say X to NAME on whatsapp"
    # "send X to NAME on whatsapp"
    # "message NAME X on whatsapp"
    # "whatsapp NAME: X"
    wa_patterns = [
        r"(?:say|send|message|tell|write)\s+(.+?)\s+to\s+(\w+)\s+on\s+whatsapp",
        r"whatsapp\s+(\w+)\s+(.+)",
        r"(?:say|send|message)\s+(\w+)\s+on\s+whatsapp\s+(.+)",
    ]
    for pat in wa_patterns:
        m = _re.search(pat, cmd)
        if m:
            g = m.groups()
            # figure out which group is contact vs message
            if "to" in command.lower() and "whatsapp" in command.lower():
                # "say X to NAME" → g[0]=message, g[1]=contact
                message = g[0].strip()
                contact = g[1].strip().title()
            else:
                contact = g[0].strip().title()
                message = g[1].strip() if len(g) > 1 else "hi"
            return "whatsapp_send", {"contact": contact, "message": message}

    # "check whatsapp" / "whatsapp unread"
    if any(p in cmd for p in ["check whatsapp", "whatsapp unread",
                               "unread whatsapp", "open whatsapp"]):
        return "whatsapp_check", {}

    # "read whatsapp from NAME"
    m = _re.search(r"read\s+whatsapp\s+(?:from\s+)?(\w+)", cmd)
    if m:
        return "whatsapp_read", {"contact": m.group(1).title()}

    # ---- YOUTUBE patterns ----
    # "play X on youtube" / "play X" / "search X on youtube"
    yt_patterns = [
        r"(?:play|listen to|watch|search for|put on)\s+(.+?)\s+on\s+youtube",
        r"youtube\s+(.+)",
        r"(?:play|listen to)\s+(.+)",
    ]
    ...
    # ---- LEETCODE patterns ----
    m = _re.search(r"(?:solve|open|do)\s+(.+?)\s+(?:on\s+)?leetcode", cmd)
    if m:
        return "leetcode", {"problem": m.group(1).strip()}
    if cmd.startswith("leetcode "):
        return "leetcode", {"problem": command[9:].strip()}

    # ---- JOB SEARCH patterns ----
    m = _re.search(r"(?:find|search|look for)\s+(.+?)\s+jobs?\s+on\s+(\w+)", cmd)
    if m:
        return "job_search", {"query": m.group(1), "platform": m.group(2)}
    m = _re.search(r"(?:find|search)\s+(.+?)\s+jobs?", cmd)
    if m:
        return "job_search", {"query": m.group(1), "platform": "naukri"}

    # ---- TELEGRAM patterns ----
    tg_patterns = [
        r"(?:say|send|message|tell)\s+(.+?)\s+to\s+(\w+)\s+on\s+telegram",
        r"telegram\s+(\w+)\s+(.+)",
    ]
...
@app.post("/chat")
def chat_command(req: CommandRequest):
    """Execute a Fury command and return output directly."""
    command = req.command.strip()
    if not command:
        raise HTTPException(status_code=400, detail="Empty command")
    ...
    try:
        ...
        if nl_type == "whatsapp_send":
            ...
            output = f"💬 WhatsApp read {contact}: {result['outcome']}"
            ...
            from platforms.job_search_agent import search_jobs
            search_jobs(nl_data["query"], nl_data.get("platform", "naukri"))
            output = f"🔍 Searching jobs: {nl_data['query']}"
        ...
        # --- EXPLICIT COMMAND ROUTING ---
            ...
            from platforms.job_search_agent import search_jobs
            search_jobs(query, platform)
            output = f"✅ Opened {platform}: {query}"
            ...
            if ":" in rest:
                ...
                parts = rest.split(" ", 1)
                contact = parts[0]
                message = parts[1] if len(parts) > 1 else "hi"
            ...
            output = f"✈️ Telegram → {contact.strip()}: {result['outcome']}"

        elif cmd in ("check telegram", "telegram unread"):
            from execution.visual_agent import run_visual_goal
            result = run_visual_goal("check telegram unread",
                                     context={"platform": "telegram"})
            output = f"✈️ Telegram: {result['outcome']}"

        elif cmd in ("check email", "read email", "gmail"):
            from execution.visual_agent import run_visual_goal
            result = run_visual_goal("open gmail inbox",
                                     context={"platform": "gmail"})
            output = f"📧 Gmail: {result['outcome']}"
            ...
            output = """FURY COMMANDS — natural language supported:

▶  YOUTUBE
   play lofi on youtube
   visual play <song> on youtube

💬  WHATSAPP
   say hi to Rishu on whatsapp
...
🔍  JOB SEARCH
   ...
   find mern developer jobs on indeed
   write cover letter React TechCorp
   draft email hr@co.com React TechCorp

✈️  TELEGRAM
   telegram John hello
   send hi to John on telegram
...
📋  HISTORY
   ...
   resume"""
            ...
            # unknown command — tell user what to try
            output = (
                f"❓ Command not recognized: '{command}'\n\n"
                f"Try:\n"
                f"  play <song> on youtube\n"
                f"  say <message> to <contact> on whatsapp\n"
                f"  leetcode <problem>\n"
                f"  search jobs <role> naukri\n"
                f"  fury help — full list"
            )
