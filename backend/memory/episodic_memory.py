...
# memory/episodic_memory.py
# STEP 117 — Deep episodic memory
# FIX: use absolute path so DB works regardless of working directory
...
DB_PATH = os.path.join(_DIR, "fury.db")
...
def save_episode(command, plan, outcome="success", verdict=None, duration_ms=0):
    ...
    try:
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            command.lower().strip(), intent,
            context_app, context_win, context_site, context_file,
            plan_json, outcome, verdict or "", duration_ms, timestamp
        ))
        conn.commit()
        conn.close()
        print("📼 Episode saved")
...
def find_similar_episode(command, limit=3):
    try:
        ...
        rows = conn.execute("""
            SELECT command, intent, context_app, context_win, context_site,
                   context_file, plan, outcome, verdict, duration_ms, timestamp
            FROM episodes ORDER BY id DESC LIMIT 50
        """).fetchall()
        conn.close()
        ...
        for ep in episodes:
            ...
            if ep["command"] == cmd_lower:
                ...
                score += 0.3
            if cur_app  and ep["context_app"]  == cur_app:  score += 0.2
            if cur_site and ep["context_site"] == cur_site: score += 0.1
            if cur_file and ep["context_file"] == cur_file: score += 0.1
            if ep["outcome"] == "failure":
                score *= 0.3
            if score > 0.2:
                ep["_score"] = round(score, 2)
...
def get_episode_stats():
    try:
        ...
        top_intents = conn.execute("""
            SELECT intent, COUNT(*) as cnt FROM episodes
            WHERE intent != '' GROUP BY intent ORDER BY cnt DESC LIMIT 5
        """).fetchall()
        conn.close()
        rate = round(success / total, 2) if total > 0 else 0.0
        return {
